...
<head>
  ...
  <style>
    ...
    .container {
      max-width: 1650px; /*  לתמוך ביותר עמודות בטבלה */
      margin: 0 auto;
      background: white;
      border-radius: 8px;
    ...
    /* אזור מסננים נוספים	 */
    /* סידור האלמנטים של פילטרים לפי תאריך, סוג טיפוח, מחיר, סטטוס הזמנה וסטטוס תשלום */
    .status-filter {
      display: flex;
      gap: 10px;
      align-items: center;
    ...
    /* רוחבים קבועים לעמודות - 11 עמודות (מסירים פרטי לקוח כי המשתמש רואה רק את ההזמנות שלו) */
    ...
    .table-header th:nth-child(5), .table-body td:nth-child(5) { width: 120px; min-width: 120px; } /* שם הכלב */
    .table-header th:nth-child(6), .table-body td:nth-child(6) { width: 120px; min-width: 120px; } /* סוג טיפוח */
    .table-header th:nth-child(7), .table-body td:nth-child(7) { width: 80px; min-width: 80px; } /* מחיר */
    .table-header th:nth-child(8), .table-body td:nth-child(8) { width: 100px; min-width: 100px; } /* סטטוס הזמנה */
    .table-header th:nth-child(9), .table-body td:nth-child(9) { width: 100px; min-width: 100px; } /* סטטוס תשלום */
    .table-header th:nth-child(10), .table-body td:nth-child(10) { width: 120px; min-width: 120px; } /* פנסיון מקושר */
    .table-header th:nth-child(11), .table-body td:nth-child(11) { width: 120px; min-width: 120px; } /* נוצר בתאריך */
    .table-header th:nth-child(12), .table-body td:nth-child(12) { width: 195px; min-width: 195px; } /* פעולות  */
    ...
    .actions {
      ...
      gap: 5px;
      justify-content: center;
      flex-wrap: wrap; /* מאפשר כפתורים לעבור לשורה חדשה במסכים קטנים */
    }
    
    /* רספונסיביות משופרת לכפתורים */
    @media (max-width: 768px) {
      .actions {
        flex-direction: column;
        gap: 3px;
...
<body>
  <div class="container">
    ...
    <div class="stats-cards">
        ...
        <div class="card">
          ...
          <div class="card-title">סה"כ כלבים שלי</div>
    ...
    <div class="table-wrapper" id="table-wrapper">
      ...
      <div class="table-body" id="table-body">
        <!--  טבלת הזמנות הטיפוח -->
        <!-- מציג את כל ההזמנות שמתקבלות מהשרת, כולל מידע כמו תאריך, שעה, כלב, סוג טיפוח -->
        <table>
          <thead>
            <tr>
              <th>#</th>
  ...
  <script>
    ...
    function fetchAppointments() {
      ...
      document.getElementById('appointments-table').innerHTML = '<tr><td colspan="12" class="text-center"><div class="loader"></div> טוען נתונים...</td></tr>';
      // שליחת בקשת GET ל־ PHP - משתמש בקובץ שמיועד למשתמש רגיל
      fetch('groomingUserPanelServer.php')
        ...
        .catch(err => {
          ...
          document.getElementById('appointments-table').innerHTML = '<tr><td colspan="12" class="text-center"><i class="fas fa-exclamation-triangle"></i> שגיאה בטעינת נתונים</td></tr>';
    ...
    function renderAppointments(data) {
      ...
      data.forEach((row, index) => {
        ...
        // יצירת כפתורי פעולה בהתאם לסטטוס ההזמנה והתשלום
        let actionButtons = '';
        
        // כפתור ביטול - רק עבור הזמנות פעילות
        ...
        // כפתור תשלום - רק עבור הזמנות פעילות שלא שולמו
        if (row.status === 'active' && row.payment_status === 'unpaid') {
          actionButtons += `
            <button class="btn btn-success" onclick="processPayment('${row.confirmation}', '${row.grooming_price}', this)" style="margin-right: 5px;">
              <i class="fas fa-credit-card"></i>
              תשלום (${row.grooming_price} ₪)
            </button>
          `;
        }
        
        // כפתור מידע עם tooltip
        actionButtons += `
          <div class="tooltip">
        ...
        /*
         ...
         סוג טיפוח 
          ...
          הזמנת שהייה מקושרת  
          תאריך יצירה
          כפתור ביטול + כפתור תשלום (אם נדרש) + כפתור מידע עם tooltip
        */
        ...
        // פונקציה לטיפול בתשלום עבור הזמנת טיפוח
        function processPayment(confirmation, price, btn) {
          // אישור מהמשתמש לפני ביצוע התשלום
          if (!confirm(`האם אתה בטוח שברצונך לשלם ${price} ₪ עבור הזמנה ${confirmation}?`)) return;
          ...
          setTimeout(() => {
            // אינטגרציה למערכת התשלומים
            // אפשר לפתוח חלון תשלום או להפנות לעמוד תשלום
            ...
            .then(data => {
              if (data.success) {
                showAlert('success', `התשלום בסך ${price} ₪ בוצע בהצלחה עבור הזמנה ${confirmation}`);
                fetchAppointments(); // רענן את רשימת ההזמנות
              } else {
                showAlert('error', 'שגיאה בביצוע התשלום: ' + data.error);
            ...
            .catch(err => {
              console.error(err);
              showAlert('error', 'שגיאה בביצוע התשלום.');
              // החזר את הכפתור למצב רגיל
              btn.innerHTML = originalContent;
              btn.disabled = false;
            });
    ...
    function cancelAppointment(confirmation, btn) {
      ...
      fetch('cancelUserAppointmentServer.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
