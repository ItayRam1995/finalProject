...
// שאילתה לשליפת ההזמנות של המשתמש הנוכחי בלבד
// רק הזמנות שהתאריך והשעה שלהן גדולים מהתאריך והשעה הנוכחיים
// ורק הזמנות טיפוח שמשויכות לקוד המשתמש הנוכחי
$query = "SELECT g.id, g.day, g.time, g.confirmation, g.created_at, 
          ...
          WHERE g.user_code = ? AND r.status != 'deleted'
            AND (
                g.day > CURDATE() 
...
// הכנת השאילתה עם פרמטר
$stmt = $conn->prepare($query);

// טיפול בשגיאה אם השאילתה לא הוכנה כראוי
if (!$stmt) {
    handleError('שגיאה בהכנת השאילתה: ' . $conn->error);
}

// קישור פרמטר קוד המשתמש
$stmt->bind_param("s", $user_code);

// הרצת השאילתה
if (!$stmt->execute()) {
    handleError('שגיאה בהרצת השאילתה: ' . $stmt->error);
}
...
// החזרת מערך ההזמנות
// נועד לספק רשימה גולמית של נתונים לצורך תצוגת טבלה של המשתמש הנוכחי בלבד
