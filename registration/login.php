...
if ($result->num_rows > 0) {
    ...
    // אם אין שדה user_code בטבלת users, אפשר להשתמש בשדה אחר כמו username או id כקוד ייחודי
    else {
        // אפשרות 1: השתמש ב-username כ-user_code
        $_SESSION['user_code'] = $user['username'];
        
        // אפשרות 2: השתמש ב-id כ-user_code
        // $_SESSION['user_code'] = (string)$user['id'];
    }

    // // אפשר גם לשמור ב-cookie למקרה שה-session יפוג תוקף
    // setcookie('user_code', $_SESSION['user_code'], time() + (86400 * 30), "/"); // תוקף: 30 יום
